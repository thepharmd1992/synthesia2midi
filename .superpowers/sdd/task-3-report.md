...
- Files changed:
  ...
  - `tests/test_main_window_layout.py`
- Behavior changed:
  - `ControlPanelQt._apply_overlay_adjustment(...)` still emits the existing `overlay_size_adjustment_requested(key_color, dimension, delta)` signal for valid clicks, but it now snapshots the targeted overlays before and after the emit and classifies the actual backend result instead of trusting the requested delta.
  - Full clean backend application keeps the local numeric counter and reset basis in sync; no-op clicks leave the visible value alone; mixed width/height outcomes and slant clamps now mark the clicked control as indeterminate with `--`.
  - Reset remains local-counted only: it emits the inverse only for nonzero integer values, and it stays silent from `0` or `--`.
  - Baseline-change and overlays-cleared resets remain intact, and the test fixture now keeps the `MainActionController` alive so the focused regressions really exercise `ControlPanelQt -> MainActionController -> OverlayManager`.
- Tests run:
  - `.venv/bin/python -m pytest tests/test_main_window_layout.py::test_overlays_tab_exposes_left_and_right_slant_controls tests/test_main_window_layout.py::test_overlays_tab_exposes_white_and_black_quick_adjust_controls -v`
  - `.venv/bin/python -m pytest tests/test_main_window_layout.py::test_overlay_quick_adjust_values_reset_when_overlay_baseline_changes tests/test_main_window_layout.py::test_overlay_quick_adjust_values_reset_when_overlays_are_cleared -v`
  - `.venv/bin/python -m pytest tests/test_main_window_layout.py::test_white_width_quick_adjust_becomes_indeterminate_when_backend_partially_applies tests/test_main_window_layout.py::test_right_slant_quick_adjust_becomes_indeterminate_when_backend_clamps_mixed_overlays -v`
  - `.venv/bin/python -m pytest tests/test_main_window_layout.py::test_overlays_tab_exposes_left_and_right_slant_controls tests/test_main_window_layout.py::test_overlays_tab_exposes_white_and_black_quick_adjust_controls tests/test_main_window_layout.py::test_overlay_quick_adjust_values_reset_when_overlay_baseline_changes tests/test_main_window_layout.py::test_overlay_quick_adjust_values_reset_when_overlays_are_cleared tests/test_main_window_layout.py::test_white_width_quick_adjust_becomes_indeterminate_when_backend_partially_applies tests/test_main_window_layout.py::test_right_slant_quick_adjust_becomes_indeterminate_when_backend_clamps_mixed_overlays -v`
  - `git diff --check`
  - `git diff --stat 0132644..HEAD -- synthesia2midi/synthesia2midi/workflows/overlay_manager.py synthesia2midi/synthesia2midi/gui/main_action_controller.py`
- Results:
  - Happy-path slant and white/black quick-adjust coverage: `2 passed in 0.30s`
  - Baseline-change and overlays-cleared reset coverage: `2 passed in 0.27s`
  - New mixed partial/clamp regressions: `2 passed in 0.27s`
  - Combined required quick-adjust suite: `6 passed in 0.32s`
  - `git diff --check`: clean
  - `git diff --stat 0132644..HEAD -- synthesia2midi/synthesia2midi/workflows/overlay_manager.py synthesia2midi/synthesia2midi/gui/main_action_controller.py`: no output
