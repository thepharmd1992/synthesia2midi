...
def move_to_top_center_safe_zone(dialog: QWidget, parent: QWidget | None = None) -> None:
    """Move a dialog high and centered so keyboard overlays stay visible."""
    if not hasattr(dialog, "frameGeometry") or not hasattr(dialog, "move"):
        return
    ...
    x = min(
        max(available.left() + ((available.width() - width) // 2), available.left()),
        max(available.left(), available.right() - width),
    )
    y = min(
        max(available.top() + 16, available.top()),
        max(available.top(), available.bottom() - height),
    )
    dialog.move(x, y)
