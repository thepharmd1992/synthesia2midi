...
class Video2MidiApp(QMainWindow, UIUpdateInterface):
    ...
    def __init__(self):
        ...
        self._auto_detect_tuning_dialog: Optional[AutoDetectTuningDialog] = None
        self._last_auto_detect_tuning_context: Optional[Dict[str, Any]] = None
        self._midi_touchup_processes: List[QProcess] = []
    ...
    def _show_conversion_complete_dialog_with_touchup(self, midi_output_path: str) -> None:
        ...
        if msg_box.clickedButton() is open_btn:
            self._open_midi_touchup_editor(midi_output_path)
    ...
    def _open_midi_touchup_editor_from_picker(self) -> None:
        ...
        if not midi_path:
            return
        self._open_midi_touchup_editor(midi_path)

    def _open_midi_touchup_editor(self, midi_path: str) -> None:
        if not os.path.exists(midi_path):
            QMessageBox.warning(self, "Touch-Up Editor", f"MIDI file not found:\n{midi_path}")
            return

        binary_path = self._resolve_midi_touchup_binary_path()
        if binary_path is None:
            self._show_midi_touchup_setup_dialog(midi_path)
            return
        ...
        self._midi_touchup_processes.append(process)

        def _on_destroyed(_obj=None, proc=process):
            self._remove_midi_touchup_process_ref(proc)

        def _on_finished(exit_code: int, _exit_status: QProcess.ExitStatus, proc=process, src_path=midi_path):
            self._handle_midi_touchup_process_finished(proc, src_path, exit_code)
        ...
        if not process.waitForStarted(2000):
            ...
            self._cleanup_midi_touchup_process(process)
            if not self._is_closing:
                QMessageBox.critical(
                    self,
                    "Touch-Up Editor Launch Failed",
                    (
                        f"Failed to start Rust touch-up editor.\n\n"
    ...
    def _resolve_midi_touchup_binary_path(self) -> Optional[str]:
        ...
        alternate = os.path.join(
            repo_root, "tools", "midi_touchup_editor_rust", "target", "release",
    ...
    def _handle_midi_touchup_process_finished(
        ...
        try:
            ...
            # The process object may already be deleted during app/window shutdown.
            logging.warning(
                "[TOUCHUP-RUST] Process handle was deleted before finish handling (source=%s)",
                source_midi_path,
            )
            self._remove_midi_touchup_process_ref(process)
            return
        ...
        failure_message = (
            message
            or f"Rust touch-up editor exited with code {exit_code}."
        )
        if not self._is_closing:
            QMessageBox.critical(
                self,
    ...
    def _cleanup_midi_touchup_process(self, process: QProcess) -> None:
        self._remove_midi_touchup_process_ref(process)
        try:
    ...
    def _remove_midi_touchup_process_ref(self, process: QProcess) -> None:
        try:
            self._midi_touchup_processes.remove(process)
        except ValueError:
            pass

    def _shutdown_midi_touchup_processes(self) -> None:
        if not self._midi_touchup_processes:
            return

        for process in list(self._midi_touchup_processes):
            self._cleanup_midi_touchup_process(process)
